...
class Solver(abc.ABC, cebra.io.HasDevice):
    ...
    def step(self, batch: cebra.data.Batch) -> dict:
        ...
        labels = torch.stack((prediction.index_ref, prediction.index_pos, prediction.Z_target), dim=1)
        self.criterion_NER = NERLoss()
        features = torch.stack((prediction.reference, prediction.positive), dim=1)
        loss = self.criterion_NER(features, labels)
        
        loss_InfoNCE, align, uniform = self.criterion(prediction.reference,
                                              prediction.positive,
                                              prediction.negative)
