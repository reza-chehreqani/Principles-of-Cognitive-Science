...
clear; clc
load('Mihili_CO_20140307_26M1_66PMd_216trial.mat')
export_file_name = 'Mihili_20140307_neural_con_dis_index_45bin.mat';
t_dur = 45 ; % median bin between idx_endTime-idx_goCueTime
...
for t = 1 : numTrials
    ...
    t_spikes_PMd_raw=PMd_spikes{t};
    t_spikes_PMd = zeros(size(t_spikes_PMd_raw));
    for i = 1:size(t_spikes_PMd_raw, 2)  % Loop over each neuron
        t_spikes_PMd(:, i) = conv(t_spikes_PMd_raw(:, i), gaussian_kernel, 'same');
    end
    t_vel=vel{t};
    ...
    if t_end<=size(t_spikes_M1,1) 
        ...
        neural_PMd_3d(:, t, :) = t_spikes_PMd(t_start:t_end, :);
    ...
    elseif t_end>size(t_spikes_M1,1)  
        ...
        disp(['Long extraction trial=',num2str(t),' miss=',num2str(t_end-size(t_spikes_M1,1))])
    end    
end    
...
neural_PMd = reshape(neural_PMd_3d, [saved_bin_num, n_neuron_PMd]);
%
save(export_file_name,'neural_M1','neural_PMd', ...
    'continuous_index','discrete_index','numTrials','excluded_short_trial');
